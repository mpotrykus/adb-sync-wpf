...
public interface IPushSafetyGuard
{
    ...
    /// <summary>Throws <see cref="PushSafetyException"/> if master is empty, or far below the best historical per-device file count. <paramref name="minimumPercent"/> is the minimum percent of that baseline master must retain.</summary>
    Task AssertSafeToPushAsync(string jobName, string masterPath, int minimumPercent, CancellationToken ct = default);
