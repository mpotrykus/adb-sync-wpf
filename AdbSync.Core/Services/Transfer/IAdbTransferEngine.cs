...
public interface IAdbTransferEngine
{
    ...
    Task<TransferResult> PullMirrorAsync(string serial, string remotePath, string localPath, IExcludeMatcher exclude, TransferPolicy? policy = null, CancellationToken ct = default);

    /// <summary>Mirrors the local source onto the device's remotePath (creates/updates/deletes to match, minus excludes).</summary>
    Task<TransferResult> PushMirrorAsync(string serial, string localPath, string remotePath, IExcludeMatcher exclude, TransferPolicy? policy = null, CancellationToken ct = default);
}
